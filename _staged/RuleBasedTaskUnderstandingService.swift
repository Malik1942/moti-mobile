...
struct RuleBasedTaskUnderstandingService: TaskUnderstandingService {
    ...
    // parseMany is handled by the protocol default, which resolves temporal per segment.

    func parse(_ input: String, temporalResolutions: [TemporalResolution] = []) async throws -> ParsedWorkItem {
        let trimmed = input.trimmingCharacters(in: .whitespacesAndNewlines)

        // Use pre-resolved temporal when it carries usable signal; otherwise run DateResolver.
        let temporal: DateResolver.TemporalParse
        let usable = temporalResolutions.filter {
            $0.resolverPath != .noSignal && $0.interpretation != .unknown
        }
        if !usable.isEmpty {
            temporal = DateResolver.TemporalParse(from: temporalResolutions, input: trimmed)
        } else {
            temporal = DateResolver.resolveTemporal(in: trimmed)
        }
